...
import io.javalin.http.InternalServerErrorResponse; // <-- CORREGIDO: Importación
...
public class AuthUtils {
    ...
    public static void requireRole(Context ctx, String requiredRole) {
        ...
        String jwtSecret = System.getProperty("JWT_SECRET"); // <-- Asegúrate de usar System.getProperty si lo seteaste así
    ...
    public static void requireOwnership(Context ctx, int resourceOwnerId) {
        String token = getTokenFromHeader(ctx);
        String jwtSecret = System.getProperty("JWT_SECRET"); // <-- Asegúrate de usar System.getProperty
        if (jwtSecret == null || jwtSecret.isEmpty()) {
            throw new InternalServerErrorResponse("JWT Secret no configurado en el servidor.");
        }
